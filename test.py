...
from datetime import datetime, timedelta
                            #Needs Updated
DB_PATH = os.path.abspath('/path/to/your/sqlite.db')
...
kraken = ccxt.kraken({
    'apiKey': '50AVpcSVD76qz2URo+tea2qJVE0/DBcoQaeaFJwojUKCh/P5FCWI43ct',
    'secret': 'SIbsAe0OA/rm+lPt4hY4DttnkZ1meZaVR0nCKnmvR7Fug23ea5f7HMOygdcL4B+wP4Xi1RmPUrcPSJ5ovPbWoQ=='
})
...
def trade_based_on_trend(conn, ticker, pair):
    ...
    if previous_trend == 'Bullish':
        buy_price = execute_trade(conn, pair, 'buy', volume)
        if buy_price:
            execute_limit_sell(conn, pair, buy_price, volume)
